...
# The demo/verification entry points that used to live here (verify_noise_power,
# filter_example and the __main__ block driving them) built a structure through
# the ANSYS bridge before exercising the acquisition chain. That bridge is gone:
# the model-based path now takes a user-supplied response, so the acquisition
# chain is exercised through the studies (and tests/test_toy_response.py)
# against a provider instead.
